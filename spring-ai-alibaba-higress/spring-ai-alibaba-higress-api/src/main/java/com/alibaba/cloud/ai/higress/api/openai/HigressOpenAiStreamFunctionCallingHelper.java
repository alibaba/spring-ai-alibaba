...
public class HigressOpenAiStreamFunctionCallingHelper {
	...
	public ChatCompletionChunk merge(ChatCompletionChunk previous, ChatCompletionChunk current) {
		...
		if (!incrementalOutput && isStreamingToolFunctionCall(current)) {
			// All of them use the operation of merging the previous choice, starting from
			// the second element of the stream, there is no information such as tool
			// name,
			// and the last finishReason = "TOOL_CALSS" tool filed = null。so all should be
			// merge
			ChunkChoice mergedChoice = merge(previousChoice0, currentChoice0);
			return new ChatCompletionChunk(id, List.of(mergedChoice), created, model, serviceTier, systemFingerprint,
					object, usage);
		}
