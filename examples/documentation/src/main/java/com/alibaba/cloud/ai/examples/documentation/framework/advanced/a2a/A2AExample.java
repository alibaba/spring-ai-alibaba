...
public class A2AExample {
    ...
    public void runDemo() throws GraphRunnerException {
        ...
        localResult.flatMap(s -> s.value("messages")).ifPresent(r ->
            System.out.println("✓ 本地调用成功，结果: " + (r.toString().length() > 100 ? r.toString().substring(0, 100) + "..." : r)));
        System.out.println();
        ...
        remoteResult.flatMap(s -> s.value("output")).ifPresent(r ->
            System.out.println("✓ 远程调用成功，结果: " + (r.toString().length() > 100 ? r.toString().substring(0, 100) + "..." : r)));
        System.out.println();
