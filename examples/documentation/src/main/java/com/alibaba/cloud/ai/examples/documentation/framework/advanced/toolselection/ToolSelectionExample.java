 ...
 *
 * 本示例演示如何使�?ToolSelectionInterceptor 进行智能工具选择�?
 *
 * 核心功能�?
 * 1. �?Agent 有多个工具时，使�?LLM 智能选择最相关的工�?
 * 2. 工具描述会自动传递给选择模型，提高选择准确�?
 * 3. 可配�?maxTools 限制每次选择的工具数�?
 * 4. 支持 alwaysInclude 确保关键工具始终可用
 *
 * 使用场景�?
 * - Agent 拥有大量工具�?5个），需要减�?token 消�?
 * - 需要提高工具选择的准确�?
 * - 不同查询需要不同的工具子集
 */
public class ToolSelectionExample {
	...
	public static void basicToolSelection() throws Exception {
		...
		ReactAgent agent = ReactAgent.builder()
			...
			.model(chatModel)
			.methodTools(travelTools)   // 自动扫描 @Tool 注解的方�?
	...
	/**
	 * 高级用法：使�?alwaysInclude 确保关键工具始终可用
	 */
	public static void toolSelectionWithAlwaysInclude() throws Exception {
		...
		// 即使查询与天气无关，weather 工具也会被包�?
		Optional<OverAllState> result = agent.invoke("帮我预订一张去上海的机�?);
	...
	public static void toolSelectionWithCustomPrompt() throws Exception {
		...
		Optional<OverAllState> result = agent.invoke("我下周要去杭州旅游，帮我看看天气和景�?);
	...
	public static void multiToolScenario() throws Exception {
		...
		ReactAgent agent = ReactAgent.builder()
			...
			.model(chatModel)
			.methodTools(travelTools, utilityTools)  // 注册多个工具�?
			.interceptors(interceptor)
			.saver(new MemorySaver())
			.build();
	...
	public static class TravelTools {
		...
		public String getWeather(
				@ToolParam(description = "城市名称，如：北京、上海、广�?) String city) {
			return String.format("%s今日天气：晴，温�?18-25°C，湿�?45%%，空气质量良好�?, city);
		}
		...
		public String searchFlights(
			...
			return String.format("找到 %s �?%s 的航班（%s）：\n" +
		...
		public String searchRestaurants(
			...
			return String.format("%s %s 餐厅推荐：\n" +
				"1. 老字号餐�?- 人均¥80 评分4.8\n" +
				"2. 网红打卡�?- 人均¥120 评分4.5\n" +
				"3. 本地特色�?- 人均¥60 评分4.7", city, cuisine);
		}
	}
	...
	public static class UtilityTools {
		...
		public String convertCurrency(
				...
				@ToolParam(description = "源货币代码，�?USD, EUR, CNY") String from,
		...
		public String translateText(
				...
				@ToolParam(description = "目标语言：中文、英文、日文、韩�?) String targetLang) {
		...
		public String calculate(
				@ToolParam(description = "数学表达式，如：100*1.1�?0+30") String expression) {
