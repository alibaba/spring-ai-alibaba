...
public class AgentVectorService {
    ...
    public List<Map<String, Object>> getAgentDatasources(Long agentId) {
        try {
            ...
            List<com.alibaba.cloud.ai.entity.AgentDatasource> agentDatasources = 
                    datasourceService.getAgentDatasources(agentId.intValue());
    ...
    private DbConfig createDbConfigFromDatasource(com.alibaba.cloud.ai.entity.Datasource datasource) {
        ...
        log.debug("Created DbConfig for datasource {}: url={}, schema={}, type={}", 
                datasource.getId(), dbConfig.getUrl(), dbConfig.getSchema(), dbConfig.getDialectType());
    ...
    public Boolean initializeSchemaForAgentWithDatasource(Long agentId, Integer datasourceId, List<String> tables) {
        try {
            String agentIdStr = String.valueOf(agentId);
            log.info("Initializing schema for agent: {} with datasource: {}, tables: {}", 
                    agentIdStr, datasourceId, tables);
            ...
            log.info("Created SchemaInitRequest for agent: {}, dbConfig: {}, tables: {}", 
                    agentIdStr, dbConfig, tables);
    ...
    public String chatWithAgent(Long agentId, String query) {
        try {
            ...
            // 创建数据库配置
            com.alibaba.cloud.ai.entity.Datasource dsEntity = datasourceService.getDatasourceById((Integer) datasource.get("id"));
            if (dsEntity == null) {
    ...
    private String processAgentQuery(String agentId, String query, DbConfig dbConfig) {
        try {
            ...
            List<org.springframework.ai.document.Document> relevantDocs = 
                vectorStoreService.getAgentVectorStoreManager().similaritySearch(agentId, query, 10);
    ...
    private boolean isGreeting(String query) {
        ...
        return lowerQuery.matches(".*(你好|hello|hi|您好|嗨|hey).*") || 
               lowerQuery.equals("你好") || 
               lowerQuery.equals("您好") ||
               lowerQuery.equals("hello") ||
