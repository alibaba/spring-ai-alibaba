...
class BaseNl2SqlServiceTest {
    ...
    void setUp() {
        ...
        gson = new Gson();
        baseNl2SqlService = new BaseNl2SqlService(
                vectorStoreService, schemaService, aiService, dbAccessor, dbConfig);
    }
    ...
    void testNl2sql() throws Exception {
        ...
        // Mock LLM 调用
        when(aiService.call(anyString()))
                .thenReturn("[\"用户\", \"年龄\"]") // extractKeywords
                .thenReturn("[\"2024-01-01\"]") // buildDateTimeExtractPrompt
                .thenReturn("[]"); // buildMixSelectorPrompt
        
        when(aiService.callWithSystemPrompt(anyString(), anyString()))
                .thenReturn("```sql\n" + expectedSql + "\n```");
    ...
    void testExpandQuestion() {
        ...
        String query = "查询用户信息";
        List<String> expectedExpansions = Arrays.asList(
                "查询用户信息",
                "获取用户数据", 
                "显示用户详情"
        );
        ...
        when(aiService.call(anyString()))
                .thenReturn(gson.toJson(expectedExpansions));
