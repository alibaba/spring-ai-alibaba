...
public class ToolsExample {
	...
	public static void accessingContext() throws GraphRunnerException {
		...
		ToolCallback accountTool = FunctionToolCallback
				...
				.inputType(AccountInfoInput.class)
				.build();
	 ...
	 * 保存用户信息输入记录类
	 */
	public record SaveUserInfoInput(
			@ToolParam(description = "The user information to save") String input
	) {}
	...
	private static ToolCallback createSaveUserInfoTool() {
		return FunctionToolCallback.builder("save_user_info", (SaveUserInfoInput input) -> {
					// 简化的实现
					return "User info saved: " + input.input();
				})
				.description("Save user information")
				.inputType(SaveUserInfoInput.class)
				.build();
	}
	...
	public record GetUserInfoInput(
			@ToolParam(description = "The user ID to look up") String userId
	) {}
	...
	private static ToolCallback createGetUserInfoTool() {
		return FunctionToolCallback.builder("get_user_info", (GetUserInfoInput input) -> {
					// 简化的实现
					return "User info for: " + input.userId();
				})
				.description("Get user information by ID")
				.inputType(GetUserInfoInput.class)
				.build();
	}
	 ...
	 * 搜索函数
	 */
	public static class SearchFunction implements Function<SearchInput, String> {
		@Override
		public String apply(SearchInput input) {
	...
	public record CalculatorInput(
			@ToolParam(description = "The mathematical expression to calculate") String expression
	) {}

	/**
	 * 计算器函数
	 */
	public static class CalculatorFunction implements Function<CalculatorInput, String> {
		@Override
		public String apply(CalculatorInput input) {
			// 简化的计算逻辑
			return "Result: " + input.expression();
		}
	}
	...
	public record SummaryInput(
			@ToolParam(description = "Input for summarizing conversation") String input
	) {}

	/**
	 * 对话摘要工具
	 */
	public static class ConversationSummaryTool implements BiFunction<SummaryInput, ToolContext, String> {

		@Override
		public String apply(SummaryInput summaryInput, ToolContext toolContext) {
	 ...
	 * 账户信息查询输入记录类
	 */
	public record AccountInfoInput(
			@ToolParam(description = "The user query for account information") String query
	) {}

	/**
	 * 账户信息工具
	 */
	public static class AccountInfoTool implements BiFunction<AccountInfoInput, ToolContext, String> {
		...
		@Override
		public String apply(AccountInfoInput input, ToolContext toolContext) {
			RunnableConfig config = (RunnableConfig) toolContext.getContext().get("config");
	 ...
	 * 带上下文的搜索工具
	 */
	public static class SearchToolWithContext implements BiFunction<SearchInput, ToolContext, String> {
		@Override
		public String apply(SearchInput input, ToolContext toolContext) {
			return "Search results for: " + input.query();
		}
	}
	 ...
	 * 带上下文的计算器函数
	 */
	public static class CalculatorFunctionWithContext implements BiFunction<CalculatorInput, ToolContext, String> {
		@Override
		public String apply(CalculatorInput input, ToolContext toolContext) {
