...
public class OpenSearchVector implements VectorStore {
    ...
    /**
     * Constructs a new instance of OpenSearchVector with the specified table name and OpenSearch configuration.
     * This constructor initializes the output fields to include CONTENT_FIELD_NAME and METADATA_FIELD_NAME.
     *
    ...
    /**
     * Constructs a new instance of OpenSearchVector with the specified table name, output fields, and OpenSearch configuration.
     * Initializes the OpenSearch client using the provided configuration.
     *
     * @param tableName The name of the table where documents will be stored.
    ...
    public void add(List<Document> documents) {
        for (Document document : documents) {
            /*
             * Document push outer structure, can add document operation structures.
             * The structure supports one or more document operations.
             */
    ...
    public List<Document> similaritySearch(SearchRequest searchRequest) {
        ...
        queryRequest.setTableName(this.tableName); // Required, the name of the table to query
    ...
    private record SearchResponseBody(
        ...
        public SearchResponseBody(JSONObject jsonObject) {
            this(
                    jsonObject.getString(ERROR_CODE_KEY),
                    jsonObject.getString(ERROR_MESSAGE_KEY),
                    jsonObject.getInteger(TOTAL_COUNT_KEY),
                    jsonObject.getJSONArray(RESULT_KEY)
            );
        }
     ...
     */
     private record ResponseBody(
         ...
         *
         * @param pushDocumentsResponseBodyString The JSON string representing the response body.
         */
        ...
        public ResponseBody(JSONObject jsonObject) {
            this(
                    jsonObject.getInteger(CODE_KEY),
                    jsonObject.getString(STATUS_KEY),
                    jsonObject.getString(ERROR_CODE_KEY),
                    jsonObject.getString(ERROR_MESSAGE_KEY)
            );
        }
    ...
    /**
     * A record class representing the result of a similarity search.
     * It contains the document ID, similarity score, content, and metadata.
     *
    ...
    /**
     * Wrapper class for interacting with OpenSearch, providing methods to upload documents,
     * delete documents, and execute search queries.
     *
     * @author fuyou.lxm
     * @since 1.0.0-M3
     */
    public static class OpenSearchClientWrapper {
         ...
         *
         * @param client      The Ha3 client used for communication with the OpenSearch service.
         ...
         *
         * @param document A map wrapped in a list representing the document to be uploaded.
         * @throws RuntimeException If the upload fails due to an error in the OpenSearch service.
         */
        public void uploadDocument(List<Map<String, ?>> document) {
            ...
            try {
                ...
                if (!responseBody.isSuccess()) {
                    ...
                    throw new RuntimeException(String.format("OpenSearch upload Document failed. " +
                            "Error code: %s. Error message: %s", errorCode, errorMsg));
                }
            } catch (Exception e) {
         ...
         *
         * @param document A map wrapped in a list representing the document to be deleted.
         * @throws RuntimeException If the deletion fails due to an error in the OpenSearch service.
         */
        public void deleteDocument(List<Map<String, ?>> document) {
            ...
            try {
                ...
                if (!responseBody.isSuccess()) {
                    String errorCode = responseBody.errorCode;
                    String errorMsg = Optional.ofNullable(responseBody.errorMessage).orElse("No error message provided");
                    throw new RuntimeException(String.format("OpenSearch delete Documents failed. " +
                            "Error code: %s. Error message: %s", errorCode, errorMsg));
                }
            } catch (Exception e) {
        ...
        /**
         * Executes a search query using the provided query request and returns the similarity results.
         *
        ...
        private SearchResponseBody getSearchResponseBody(SearchResponse searchResponse) {
            ...
            if (responseBody.hasError()) {
                ...
                throw new RuntimeException(String.format("OpenSearch inferenceQuery failed. " +
                        "Error code: %s. Error message: %s", errorCode, errorMsg));
            }
    ...
    /**
     * A utility class for parsing search results from a response body into a list of {@link SimilarityResult} objects.
     * This class handles the extraction of relevant information such as document ID, content, score, and metadata from the JSON response.
     *
     * @author fuyou.lxm
     * @since 1.0.0-M3
     */
    private static class SearchResultParser {
        ...
        /**
         * Parses the search response body and returns a list of {@link SimilarityResult} objects.
         *
         * @param responseBody The search response body containing the results.
         * @return A list of {@link SimilarityResult} objects extracted from the response body.
         */
        ...
        /**
         * Parses a single JSON object representing a document into a {@link SimilarityResult} object.
         *
         ...
         *
         * @param jsonDocument The JSON object containing the document details.
         * @return A map of metadata extracted from the document, or an empty map if not found.
         */
