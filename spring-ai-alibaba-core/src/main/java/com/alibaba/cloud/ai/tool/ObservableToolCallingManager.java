...
public class ObservableToolCallingManager implements ToolCallingManager {
  ...
  // TODO Mandatory Convention as ARMS implementation until the Spring AI project officially supports for observation
  private final ArmsToolCallingObservationConvention observationConvention = DEFAULT_OBSERVATION_CONVENTION;

  public ObservableToolCallingManager(ObservationRegistry observationRegistry, ToolCallbackResolver toolCallbackResolver,
  ...
  public ToolExecutionResult executeToolCalls(Prompt prompt, ChatResponse chatResponse) {
    ...
    // According to org.springframework.ai.openai.api.OpenAiStreamFunctionCallingHelper.merge, we
    //  couldn't handle more than one tool calls in a single completion because of missing the
  ...
  private static AssistantMessage mergeToolCalls(AssistantMessage assistantMessage) {
    ...
    return new AssistantMessage(assistantMessage.getText(), assistantMessage.getMetadata(), toolCalls, assistantMessage.getMedia());
  }
  ...
  private InternalToolExecutionResult executeToolCall(Prompt prompt, AssistantMessage assistantMessage,
    ...
    for (AssistantMessage.ToolCall toolCall : assistantMessage.getToolCalls()) {
      ...
      String toolResult = ArmsToolCallingObservationDocumentation.EXECUTE_TOOL_OPERATION.observation(
