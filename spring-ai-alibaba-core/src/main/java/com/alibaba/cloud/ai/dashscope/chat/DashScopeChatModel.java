...
/**
 * {@link ChatModel} implementation for {@literal Alibaba DashScope} backed by {@link Generation}.
 *
 * @author yuluo
...
public class DashScopeChatModel extends AbstractToolCallSupport implements ChatModel {
    ...
    public DashScopeChatModel(DashScopeApi dashscopeApi) {
        this(dashscopeApi,
                DashScopeChatOptions.builder().withModel(DashScopeApi.DEFAULT_CHAT_MODEL).withTemperature(0.7f)
                        .build());
    }
    ...
    public ChatResponse call(Prompt prompt) {
        ...
        ResponseEntity<ChatCompletion> completionEntity = this.retryTemplate.execute(
                ctx -> this.dashscopeApi.chatCompletionEntity(request));
    ...
    public Flux<ChatResponse> stream(Prompt prompt) {
        ...
        Flux<ChatCompletionChunk> completionChunks = this.retryTemplate.execute(
                ctx -> this.dashscopeApi.chatCompletionStream(request));
        ...
        Flux<ChatResponse> chatResponse = completionChunks.map(this::chunkToChatCompletion)
                .switchMap(chatCompletion -> Mono.just(chatCompletion).map(chatCompletion2 -> {
                    try {
                        @SuppressWarnings("null") String requestId = chatCompletion2.requestId();
    ...
    private ChatResponseMetadata from(ChatCompletion result) {
        ...
        return ChatResponseMetadata.builder().withId(result.requestId())
                .withUsage(DashScopeAiUsage.from(result.usage())).withModel("").build();
    }
            ...
            if (message.getMessageType() == MessageType.USER || message.getMessageType() == MessageType.SYSTEM) {
                ...
                if (message instanceof UserMessage userMessage) {
                    if (!CollectionUtils.isEmpty(userMessage.getMedia())) {
                        ...
                        contentList.addAll(userMessage.getMedia().stream().map(media -> new MediaContent(
                ...
                return List.of(
                        new ChatCompletionMessage(assistantMessage.getContent(), ChatCompletionMessage.Role.ASSISTANT,
                                null, null, toolCalls));
