...
public class MockWeatherServiceWithToolContext implements
		BiFunction<MockWeatherServiceWithToolContext.Request, ToolContext, MockWeatherServiceWithToolContext.Response> {

	@Override
	public Response apply(Request request, ToolContext toolContext) {
		Object o = toolContext.getContext().get("location");
		System.out.println(o);

		if (request.city().contains("杭州")) {
			return new Response(String.format("%s%s晴转多云, 气温32摄氏度。", request.date(), request.city()));
		}
		if (request.city().contains("成都")) {
			return new Response(String.format("%s%s晴转多云, 气温32摄氏度。", request.date(), o));
		}
