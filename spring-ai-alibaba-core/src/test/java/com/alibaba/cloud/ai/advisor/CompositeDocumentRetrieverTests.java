...
class CompositeDocumentRetrieverTests {
	...
	void testRealEnterpriseScenario() {
		...
		assertThat(techResults).anyMatch(doc -> 
			doc.getText().contains("Spring AI") && doc.getScore() > 0.0);
		...
		assertThat(securityResults).anyMatch(doc -> 
			doc.getText().contains("安全") && doc.getScore() > 0.0);
		...
		assertThat(productResults).anyMatch(doc -> 
			doc.getText().contains("通义千问") && doc.getScore() > 0.0);
	}
	...
	private DocumentRetriever createRealPolicyRetriever() {
		List<String> policies = Arrays.asList(
			"代码审查规范：所有代码必须经过至少一名高级工程师审查才能合并到主分支。审查内容包括代码质量、安全性、性能和可维护性。",
	...
	private DocumentRetriever createRealProductRetriever() {
		List<String> products = Arrays.asList(
			"通义千问大语言模型：支持多轮对话、代码生成、文本总结、翻译等功能。提供 API 接口和 SDK，支持流式输出，具有强大的中文理解能力。",
	...
	private DocumentRetriever createMockRetrieverWithRealData(String source, String category, List<String> documents, String author) {
		return new DocumentRetriever() {
			...
			public List<Document> retrieve(Query query) {
				...
				for (int i = 0; i < documents.size(); i++) {
					...
					metadata.put("lastUpdated", "2025-07-" + (20 + i % 5) + "T" + 
						String.format("%02d", 9 + i % 12) + ":00:00Z");
	...
	private boolean isSemanticMatch(String queryWord, String text) {
		if ("安全".equals(queryWord) && text.contains("保护")) return true;
		if ("保护".equals(queryWord) && text.contains("安全")) return true;
		if ("方案".equals(queryWord) && text.contains("政策")) return true;
		if ("政策".equals(queryWord) && text.contains("方案")) return true;
		if ("企业级".equals(queryWord) && text.contains("企业")) return true;
		if ("企业".equals(queryWord) && text.contains("企业级")) return true;
		if ("管理".equals(queryWord) && text.contains("配置")) return true;
		if ("配置".equals(queryWord) && text.contains("管理")) return true;
		if ("最佳实践".equals(queryWord) && text.contains("规范")) return true;
		if ("规范".equals(queryWord) && text.contains("最佳实践")) return true;
		if ("集成".equals(queryWord) && text.contains("整合")) return true;
		if ("整合".equals(queryWord) && text.contains("集成")) return true;
		if ("模型".equals(queryWord) && text.contains("框架")) return true;
		if ("框架".equals(queryWord) && text.contains("模型")) return true;
		return false;
	}
