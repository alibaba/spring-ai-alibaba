...
public class BaiduSearchService implements Function<BaiduSearchService.Request, BaiduSearchService.Response> {
	...
	public BaiduSearchService.Response apply(BaiduSearchService.Request request) {
		...
		try {
			List<SearchResult> results = parseHtml(url);
	...
	private List<SearchResult> parseHtml(String url) {
		try {
			...
			for (Element div : divContents) {
				...
				try {
					if (div.hasClass("xpath-log")) {
						...
						if (div.selectFirst(".c-abstract") != null) {
							abstractText = div.selectFirst(".c-abstract").text().trim();
						}
					...
					else if (div.hasClass("result-op")) {
						if (div.selectFirst("h3") != null) {
							title = div.selectFirst("h3").text().trim();
						}
						else {
						...
						if (div.attr("tpl").equals("se_com_default")) {
							if (div.selectFirst("h3") != null) {
								title = div.selectFirst("h3").text().trim();
							}
							else {
