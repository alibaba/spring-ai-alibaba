...
public class PlanTemplateController {
	...
	/**
	 * 直接执行JSON计划
	 * 这个方法可以直接接收JSON格式的计划内容，生成新的计划ID并执行
	 * 
	 * @param request 包含计划JSON内容的请求
	 * @return 执行结果，包含新生成的计划ID
	 */
	@PostMapping("/execute-plan")
	public ResponseEntity<Map<String, Object>> executePlanFromJson(@RequestBody Map<String, String> request) {
		String planJson = request.get("planJson");
		String description = request.get("description"); // 可选的计划描述
		
		if (planJson == null || planJson.trim().isEmpty()) {
			return ResponseEntity.badRequest().body(Map.of("error", "计划JSON数据不能为空"));
		}
		...
		try {
			...
			planTemplateService.savePlanTemplate(
				planTemplateId, 
				title, 
				description != null ? description : "直接执行JSON计划", 
				planJson
			);
			
			logger.info("已创建新计划: {}", planTemplateId);
			...
			context.setPlanId(planTemplateId);
			context.setNeedSummary(true); // 需要生成摘要
			context.setUserRequest("直接执行JSON计划: " + title);
			...
			CompletableFuture.runAsync(() -> {
				try {
					// 执行完整的计划流程
					planningCoordinator.executePlan(context);
					logger.info("计划执行成功: {}", planTemplateId);
				}
				catch (Exception e) {
					logger.error("执行计划失败", e);
				}
			...
			// 返回任务ID及初始状态
			Map<String, Object> response = new HashMap<>();
			response.put("planId", planTemplateId);
			response.put("status", "processing");
			response.put("message", "计划执行请求已提交，正在处理中");
			
			return ResponseEntity.ok(response);
		} catch (Exception e) {
			logger.error("执行JSON计划失败", e);
			return ResponseEntity.internalServerError().body(Map.of("error", "执行JSON计划失败: " + e.getMessage()));
		}
	}
}
