...
/**
 * World Bank Data Service for accessing development indicators and country data.
 * Provides access to World Bank's development data through their open API.
...
public class WorldBankDataService 
	...
	private String buildEndpoint(Request request) {
		...
		if (StringUtils.hasText(request.country()) && StringUtils.hasText(request.indicator())) {
			...
			endpoint.append("/country/").append(request.country())
					.append("/indicator/").append(request.indicator());
	...
	public record Request(
			...
			@JsonPropertyDescription("Search query - can be indicator code (e.g., 'SP.POP.TOTL'), country code (e.g., 'CHN'), or search terms") 
			String query,
			
			@JsonProperty(value = "country") 
			@JsonPropertyDescription("Country code (e.g., 'CHN' for China, 'USA' for United States, 'all' for all countries)") 
			String country,
			
			@JsonProperty(value = "indicator") 
			@JsonPropertyDescription("Indicator code (e.g., 'SP.POP.TOTL' for Population, 'NY.GDP.MKTP.CD' for GDP)") 
			String indicator,
			
			@JsonProperty(value = "dateRange") 
			@JsonPropertyDescription("Date range for data (e.g., '2020', '2018:2022', 'YTD:2023')") 
			String dateRange,
			
			@JsonProperty(value = "queryType", defaultValue = "data") 
			@JsonPropertyDescription("Type of query: 'data' for indicator data, 'metadata' for indicator information, 'country' for country info") 
			String queryType,
			
			@JsonProperty(value = "page", defaultValue = "1") 
			@JsonPropertyDescription("Page number for pagination") 
			Integer page,
			
			@JsonProperty(value = "perPage", defaultValue = "10") 
			@JsonPropertyDescription("Number of results per page (max 100)") 
			Integer perPage,
			
			@JsonProperty(value = "mrv") 
			@JsonPropertyDescription("Most recent values count (e.g., 5 for last 5 available data points)") 
			Integer mrv
