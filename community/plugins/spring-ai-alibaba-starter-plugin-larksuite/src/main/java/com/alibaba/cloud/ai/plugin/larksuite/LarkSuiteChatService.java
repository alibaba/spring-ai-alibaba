...
public class LarkSuiteChatService implements Function<LarkSuiteChatService.IMChatRequest, CreateMessageResp> {
    ...
    public CreateMessageResp apply(IMChatRequest request) {
        ...
        CreateMessageReq req = CreateMessageReq.newBuilder().createMessageReqBody(CreateMessageReqBody.newBuilder().receiveId(request.receiveId()).msgType(request.msgType()).content(request.content()).uuid(request.uuid()).build()).build();
