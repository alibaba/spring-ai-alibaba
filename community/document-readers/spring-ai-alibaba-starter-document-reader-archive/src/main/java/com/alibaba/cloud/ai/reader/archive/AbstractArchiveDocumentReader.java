 ...
 * @author aruato
 */
public abstract class AbstractArchiveDocumentReader implements DocumentReader {
	...
	public List<Document> get() {
		try (InputStream raw = resource.getInputStream(); ArchiveInputStream ais = createArchiveStream(raw, charset)) {
			...
			while ((entry = ais.getNextEntry()) != null) {
				...
				byte[] entryContent = readEntryContent(ais);

				// Parse entry content and enrich with metadata
				try (InputStream bais = new ByteArrayInputStream(entryContent)) {
					for (Document doc : parser.parse(bais)) {
	...
	/**
	 * Reads the full byte content of the current entry from the ArchiveInputStream.
	 */
	private byte[] readEntryContent(ArchiveInputStream ais) throws IOException {
	 ...
	 * format.
	 */
	protected abstract ArchiveInputStream createArchiveStream(InputStream in, String charset) throws IOException;
