...
/// LogFactory.getLog(DashScopeAutoConfigurationIT.class);
//
...
/// context.getBean(DashScopeSpeechSynthesisModel.class);
...
// this.contextRunner.run(context -> {
// DashScopeChatModel chatModel = context.getBean(DashScopeChatModel.class);
// Flux<ChatResponse> responseFlux = chatModel.stream(new Prompt(new
/// UserMessage("Hello")));
...
// String response =
/// Objects.requireNonNull(responseFlux.collectList().block()).stream().map(chatResponse
/// -> {
...
/// context.getBean(DashScopeEmbeddingModel.class);
//
