		...
		if (result != null) {
			...
			if (output != null) {
				...
				if (StringUtils.isBlank(responseContent)) {
					responseContent = output.hasToolCalls() ? output.getToolCalls()
						.stream()
						.map(AssistantMessage.ToolCall::arguments)
						.collect(Collectors.joining(System.lineSeparator())) : "发生了异常错误";
				}
				String input = promptTemplate.render(Map.of(SELECTION, selections, "input", responseContent));
				chatResponseBuilder
					.generations(List.of(new Generation(new AssistantMessage(this.router.apply(input), Map.of()))));
			}
		}
