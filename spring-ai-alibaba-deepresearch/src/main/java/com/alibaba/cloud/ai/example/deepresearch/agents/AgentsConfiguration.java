...
public class AgentsConfiguration {
	...
	public AgentsConfiguration(@Value("classpath:prompts/researcher.md") Resource researcherPrompt,
			...
			@Value("classpath:prompts/reporter.md") Resource reporterPrompt, ApplicationContext context,
			Optional<Map<String, AsyncMcpToolCallbackProvider>> asyncProviders,
			Optional<Map<String, SyncMcpToolCallbackProvider>> syncProviders, PythonCoderProperties coderProperties,
			PlannerTool plannerTool, AgentModelsConfiguration agentModelsConfiguration,
	...
	private String[] getAvailableTools(String... toolNames) {
		return toolNames == null ? new String[0]
				: Arrays.stream(toolNames).filter(context::containsBean).toArray(String[]::new);
	}
	...
	private ChatClient coordinatorAgentInternal(ChatClient.Builder builder, PlannerTool plannerTool) {
		return builder.defaultOptions(ToolCallingChatOptions.builder().internalToolExecutionEnabled(false).build())
	...
	public ChatClient buildResearchAgent(String modelName) {
		...
		return buildAgent(builder, researcherPrompt, getMcpToolCallbacks("researchAgent"),
				(Object[]) getAvailableTools(JinaCrawlerConstants.TOOL_NAME));
	}

	public ChatClient buildCoderAgent(String modelName) {
		ChatClient.Builder builder = agentModelsConfiguration.builderChatClient(modelName).mutate();
		return buildAgent(builder, coderPrompt, getMcpToolCallbacks("coderAgent"), new PythonReplTool(coderProperties));
	}
