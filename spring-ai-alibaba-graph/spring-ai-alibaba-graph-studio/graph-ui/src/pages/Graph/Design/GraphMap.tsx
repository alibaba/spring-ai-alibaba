...
import {Background, MiniMap, ReactFlow, useReactFlow, useStoreApi, useViewport} from '@xyflow/react';
import React, {
    memo,
...
const LayoutFlow = memo(() => {
    ...
    const onNodesChangeHook = useCallback(
        (changes: any) => {
            if (graphStore.readonly)
                return;
    ...
    const onNodeClickHook = useCallback(
...
export default memo(() => {
    ...
    }, [screenToFlowPosition, graphStore.mousePosition, setNodes]);

    const addNode = useCallback((event: any) => {
