...
public interface NodeDataConverter<T extends NodeData> {
    ...
    /**
     * Generate a readable variable name prefix for this node, such as "http1", "llm2", and so on
     * @param count
     * @return friendly varName
     */
    default String generateVarName(int count) {
        throw new UnsupportedOperationException(getClass().getSimpleName()
                + " must implement generateVarName");
    }

    /**
     * After parseMapData is complete and varName is injected, call:
     * Used to override the default outputKey based on varName and refresh the list of outputs
     * @param nodeData {@link NodeData}
     * @param varName
     */
