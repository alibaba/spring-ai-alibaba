...
public class LLMNodeSection implements NodeSection {
	...
	public String render(Node node) {
		...
		String msgs = d.getPromptTemplate().stream().map(pt -> {
			// 角色对应不同的 Message 类
			String cls = pt.getRole().equals("system") ? "SystemMessage" : "UserMessage";
		...
		if (d.getMemoryConfig() != null && d.getMemoryConfig().getEnabled()) {
			memConfig = String.format("    .memoryConfig(new MemoryConfig(%b, %d))%n",
					d.getMemoryConfig().getWindowEnabled(), d.getMemoryConfig().getWindowSize());
		}

		// 最终拼装
		return String.format("// —— LlmNode [%s] ——%n" + "LlmNode %1$sNode = LlmNode.builder()%n"
				+ "    .chatClient(chatClient)%n" + "    .messages(List.of(%n%s%n    ))%n" + "%s" + // memoryConfig
																									// 段，可为空
				"    .build();%n" + "stateGraph.addNode(\"%s\", AsyncNodeAction.node_async(%1$sNode));%n%n", id, msgs,
				memConfig, id);
	}
