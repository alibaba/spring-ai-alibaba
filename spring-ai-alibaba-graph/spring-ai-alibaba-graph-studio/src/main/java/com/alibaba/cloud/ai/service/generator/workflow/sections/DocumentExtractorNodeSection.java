...
public class DocumentExtractorNodeSection implements NodeSection {
	...
	public String render(Node node) {
		...
		return String.format("// —— DocumentExtractor [%s] ——%n"
				+ "DocumentExtractorNode %1$sNode = DocumentExtractorNode.builder()%n"
				+ "    .outputSchema(DocumentExtractorNodeData.DEFAULT_OUTPUT_SCHEMA)%n" + "    .build();%n"
				+ "stateGraph.addNode(\"%s\", AsyncNodeAction.node_async(%1$sNode));%n%n", id, id);
	}
