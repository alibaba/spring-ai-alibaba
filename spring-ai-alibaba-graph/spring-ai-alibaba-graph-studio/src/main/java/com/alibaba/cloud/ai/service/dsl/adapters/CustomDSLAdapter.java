...
public class CustomDSLAdapter extends AbstractDSLAdapter {
    ...
    public Workflow mapToWorkflow (Map<String, Object> data) {
        ...
        if (specMap.containsKey("workflowVars")) {
            ...
            List<Variable> workflowVars = variables.stream().map(variable -> objectMapper.convertValue(variable, Variable.class)).toList();
        ...
        if (specMap.containsKey("envVars")) {
            ...
            List<Variable> envVars = variables.stream().map(variable -> objectMapper.convertValue(variable, Variable.class)).toList();
    ...
    private Node constructNode (Map<String, Object> nodeMap) {
        ...
        NodeType nodeType = NodeType.fromValue(node.getType()).orElseThrow(() -> new NotImplementedException("Unsupported Node Type: " + node.getType()));
