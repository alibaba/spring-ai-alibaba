        ...
        DIFY(new DialectConverter<>() {
            ...
            public LLMNodeData parse(Map<String, Object> data) {
                ...
                if (sel != null && sel.size() == 2) {
                    nd.setInputs(
                            Collections.singletonList(new VariableSelector(sel.get(0), sel.get(1)))
                    );
                }
                ...
                if (prompts != null) {
                    List<LLMNodeData.PromptTemplate> pt = prompts.stream()
                            .map(m -> new LLMNodeData.PromptTemplate(
                                    (String) m.get("role"),
                                    (String) m.get("text")
                            ))
                            .collect(Collectors.toList());
                ...
                if (tList != null) {
                    List<LLMNodeData.ToolCallback> cbObjs = tList.stream()
                            .map(m -> new LLMNodeData.ToolCallback(
                                    (String) m.get("name"),
            ...
            public Map<String, Object> dump(LLMNodeData nd) {
                ...
                if (nd.getPromptTemplate() != null && !nd.getPromptTemplate().isEmpty()) {
                    List<Map<String, Object>> pt = nd.getPromptTemplate().stream()
                            .map(t -> {
                ...
                if (nd.getMessages() != null && !nd.getMessages().isEmpty()) {
                    List<Map<String, Object>> ml = nd.getMessages().stream()
                            .map(msg -> {
                                Map<String, Object> entry = new LinkedHashMap<>();
                ...
                if (nd.getAdvisors() != null && !nd.getAdvisors().isEmpty()) {
                    List<Map<String, Object>> al = nd.getAdvisors().stream()
                            .map(a -> {
                                Map<String, Object> entry = new LinkedHashMap<>();
                ...
                if (nd.getToolCallbacks() != null && !nd.getToolCallbacks().isEmpty()) {
                    List<Map<String, Object>> tl = nd.getToolCallbacks().stream()
                            .map(tc -> {
                                Map<String, Object> entry = new LinkedHashMap<>();
