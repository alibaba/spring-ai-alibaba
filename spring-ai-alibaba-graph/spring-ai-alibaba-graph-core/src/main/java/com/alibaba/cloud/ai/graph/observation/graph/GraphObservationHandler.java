...
/**
 * Handler for graph observation events.
 * Processes graph observation contexts and generates metrics for monitoring.
 *
 * @author XiaoYunTao
 * @since 2025/6/29
 */
public class GraphObservationHandler implements ObservationHandler<GraphObservationContext> {
    ...
    /**
     * Handles successful graph observation completion.
     * Logs graph information and generates success metrics.
     */
    @Override
    public void onStop(GraphObservationContext context) {
        logger.info("Graph graphName: {} state: {} output : {}", context.getGraphName(), context.getState().toString(), context.getOutput().toString());
        GraphMetricsGenerator.generate(context, meterRegistry, true);
    }

    /**
     * Handles graph observation errors.
     * Logs graph information and generates error metrics.
     */
    @Override
