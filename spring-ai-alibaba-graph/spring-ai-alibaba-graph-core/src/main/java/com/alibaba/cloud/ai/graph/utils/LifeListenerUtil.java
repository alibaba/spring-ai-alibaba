...
/**
 * Utility class for processing graph lifecycle listeners.
 * This class provides methods to handle various lifecycle events
 * such as start, end, error, and node before/after events.
 */
public class LifeListenerUtil {
    private static final Logger log = LoggerFactory.getLogger(LifeListenerUtil.class);

    /**
     * Process graph lifecycle listeners in LIFO (Last In, First Out) order.
     * This method recursively processes all listeners in the deque, invoking
     * the appropriate callback method based on the scene parameter.
     *
     ...
     */
    public static void processListenersLIFO(String currentNodeId,
                                          Deque<GraphLifecycleListener> listeners,
