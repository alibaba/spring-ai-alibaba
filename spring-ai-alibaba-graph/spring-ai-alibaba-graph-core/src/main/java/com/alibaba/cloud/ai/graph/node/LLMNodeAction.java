...
public class LLMNodeAction<State extends AgentState> extends AbstractNode implements NodeAction<State> {
     ...
     */
    @Override
    public Map<String, Object> apply (@NotNull State state) throws Exception {

        List<Message> messages = state.value(MESSAGES_KEY, new ArrayList<>());
        List<Generation> generations = chatClient.prompt().user(s -> s.params(state.data())).messages(messages).call().chatResponse().getResults();
