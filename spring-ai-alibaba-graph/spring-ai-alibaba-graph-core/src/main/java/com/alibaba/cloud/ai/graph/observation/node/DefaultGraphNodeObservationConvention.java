...
/**
 * Default implementation of GraphNodeObservationConvention.
 * Provides standard observation conventions for graph node operations including
 * contextual naming and key-value generation for both low and high cardinality metrics.
 *
 * @author XiaoYunTao
 * @since 2025/6/28
 */
public class DefaultGraphNodeObservationConvention implements GraphNodeObservationConvention {
    ...
    /**
     * Constructs a new DefaultGraphNodeObservationConvention with the default operation name.
     */
    ...
    /**
     * Generates a contextual name based on the node observation context.
     * If the context has a name, it appends it to the default operation name.
     *
     * @param context the graph node observation context
     * @return the contextual name, or the default operation name if no context name is available
     */
    @Override
    @Nullable
    ...
    /**
     * Generates low cardinality key-values for graph node observations.
    ...
    public KeyValues getLowCardinalityKeyValues(GraphNodeObservationContext context) {
        return KeyValues.of(
                KeyValue.of(GraphNodeObservationDocumentation.LowCardinalityKeyNames.SPRING_AI_ALIBABA_KIND, SpringAiAlibabaKind.GRAPH_NODE.getValue()),
                KeyValue.of(GraphNodeObservationDocumentation.LowCardinalityKeyNames.GRAPH_NODE_NAME, context.getNodeName()),
                KeyValue.of(GraphNodeObservationDocumentation.LowCardinalityKeyNames.GRAPH_EVENT, context.getEvent())
        );
    }

    /**
     * Generates high cardinality key-values for graph node observations.
    ...
    public KeyValues getHighCardinalityKeyValues(GraphNodeObservationContext context) {
        return KeyValues.of(
                KeyValue.of(GraphNodeObservationDocumentation.HighCardinalityKeyNames.GRAPH_NODE_STATE, context.getState().toString()),
                KeyValue.of(GraphNodeObservationDocumentation.HighCardinalityKeyNames.GRAPH_NODE_OUTPUT, context.getOutput().toString())
        );
    }
}
