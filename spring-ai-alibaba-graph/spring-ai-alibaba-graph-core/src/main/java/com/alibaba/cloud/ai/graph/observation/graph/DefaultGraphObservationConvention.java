...
/**
 * Default implementation of GraphObservationConvention.
 * Provides standard observation conventions for graph operations with configurable naming.
 *
 * @author XiaoYunTao
 * @since 2025/6/29
 */
public class DefaultGraphObservationConvention implements GraphObservationConvention {
    ...
    /**
     * Generates a contextual name for the observation.
     * Combines the operation name with the graph name if available.
     */
    @Override
    @Nullable
    ...
    /**
     * Provides low cardinality key values for metrics.
     * Includes graph kind and name for grouping and filtering.
     */
    @Override
    public KeyValues getLowCardinalityKeyValues(GraphObservationContext context) {
        return KeyValues.of(
                KeyValue.of(GraphObservationDocumentation.LowCardinalityKeyNames.SPRING_AI_ALIBABA_KIND, SpringAiAlibabaKind.GRAPH.getValue()),
                KeyValue.of(GraphObservationDocumentation.LowCardinalityKeyNames.GRAPH_NAME, context.getGraphName())
        );
    }

    /**
     * Provides high cardinality key values for detailed analysis.
     * Includes graph state and output information.
     */
    @Override
    public KeyValues getHighCardinalityKeyValues(GraphObservationContext context) {
        return KeyValues.of(
                KeyValue.of(GraphObservationDocumentation.HighCardinalityKeyNames.GRAPH_NODE_STATE, context.getState().toString()),
                KeyValue.of(GraphObservationDocumentation.HighCardinalityKeyNames.GRAPH_NODE_OUTPUT, context.getOutput().toString())
        );
    }
}
