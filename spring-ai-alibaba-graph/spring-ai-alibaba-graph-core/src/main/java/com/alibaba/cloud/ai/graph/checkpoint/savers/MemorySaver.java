...
public class MemorySaver implements BaseCheckpointSaver {
    ...
    @Override
    public Optional<Checkpoint> getByNodeId(RunnableConfig config, String nodeId) {
        final LinkedList<Checkpoint> checkpoints = getCheckpoints(config);
        r.lock();
        try {
            return checkpoints.stream().filter(checkpoint -> checkpoint.getNodeId().equals(nodeId)).findFirst();
        } finally {
            r.unlock();
        }
    }
