...
"""
Spring AI Alibaba Chinese Content Checker - Local Version
Tool for checking Chinese content in Java and frontend code
"""
...
class ChineseContentChecker:
    ...
    def report_issues(self) -> None:
        ...
        print(f"❌ Found {len(self.issues)} Chinese content issues")
        ...
        for file_path, file_issues in files_with_issues.items():
            print(f"\nFile contains {len(file_issues)} Chinese content issues: {file_path}")
        ...
        print("1. Change Chinese comments to English comments")
        print("2. Extract Chinese strings to resource files or configuration files")
        print("3. Change Chinese identifiers to English identifiers")
        print("4. For test data, consider using English or placeholders")

def main():
    parser = argparse.ArgumentParser(description='Check Chinese content in Java and frontend code')
    parser.add_argument('--dirs', '-d',
                       nargs='+',
                       default=['src/main/java', 'ui-vue3/src'],
    ...
    try:
        ...
        if args.fail_on_found and not is_clean:
            print(f"Check failed: Found {len(checker.issues)} Chinese content issues")
            return 1
