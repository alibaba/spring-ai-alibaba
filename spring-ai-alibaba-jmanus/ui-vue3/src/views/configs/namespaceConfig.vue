...
const selectNamespace = async (namespace: Namespace) => {
  try {
    const detailedNamespace = await NamespaceApiService.getPromptById(namespace.id)
...
const handleAddNamespace = async () => {
  ...
  try {
    ...
    const createdNamespace = await NamespaceApiService.createPrompt(namespaceData)
...
const handleSave = async () => {
  ...
  try {
    const savedNamespace = await NamespaceApiService.updatePrompt(
...
const showAddNamespaceModal = () => {
  newNamespace.name = ''
  newNamespace.promptDescription = ''
  newNamespace.promptContent = ''
