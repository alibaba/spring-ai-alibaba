        ...
        <div class="modal-container" @click.stop>
          ...
          <div class="modal-content">
            <form @submit.prevent="handleSave" class="task-form">
              ...
              <div v-if="task?.nextExecutionTime" class="form-group">
                <label class="form-label">{{ $t('cronTask.nextExecution') }}</label>
                <div class="form-info">{{ formatTime(task.nextExecutionTime) }}</div>
              </div>
            </form>
          </div>
...
watch(
  () => props.task,
  (newTask) => {
    console.log('TaskDetailModal received task:', newTask)
    if (newTask) {
      formData.value = {
        cronName: newTask.name || newTask.cronName || '',
        cronTime: newTask.cronExpression || newTask.cronTime || '',
        planDesc: newTask.description || newTask.planDesc || '',
        status: newTask.status ?? 1,
      }
      console.log('TaskDetailModal formData updated:', formData.value)
    }
  },
  { immediate: true }
)
