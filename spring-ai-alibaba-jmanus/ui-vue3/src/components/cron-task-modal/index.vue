              ...
              <div v-for="task in cronTasks" :key="task.id || ''" class="task-item" @click="showTaskDetail(task)">
                <div class="task-main">
                  <div class="task-info">
                    ...
                    <div class="task-description">{{ task.planDesc || task.description }}</div>
                    <div class="task-time">
...
// 计算活跃和非活跃任务数量（如果需要在模板中使用，可以取消注释）
// const activeTasksCount = computed(() => {
//   return cronTasks.value.filter(task => task.status === 0).length
// })

// const inactiveTasksCount = computed(() => {
//   return cronTasks.value.filter(task => task.status === 1).length
// })
...
const executeTask = async (taskId: string | number) => {
  ...
  try {
    ...
    // Use planDesc or description as the task content
    const taskContent = task.planDesc || task.description || task.cronName || ''

    if (taskContent.trim()) {
      console.log('[CronTaskModal] Setting task to store:', taskContent)
      ...
      console.log('[CronTaskModal] Navigating to direct page with chatId:', chatId)
      ...
      console.log('[CronTaskModal] Navigation to direct page completed')
    } else {
      console.warn('[CronTaskModal] No task content found for task:', task)
    }
  } catch (error) {
...
const showTaskDetail = (task: CronConfig) => {
  console.log('showTaskDetail called with task:', task)
  selectedTask.value = { ...task }
  console.log('selectedTask.value set to:', selectedTask.value)
  showDetail.value = true
}

const editTask = (task: CronConfig) => {
  selectedTask.value = { ...task }
  showDetail.value = true
