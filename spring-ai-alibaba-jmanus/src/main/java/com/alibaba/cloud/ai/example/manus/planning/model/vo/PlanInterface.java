...
public interface PlanInterface {
	...
	/**
	 * 获取计划的最外层父计划ID（rootPlanId）。
	 * rootPlanId means the ID of the outermost/root parent plan for the entire execution plan.
	 * 在主计划和所有子计划中，rootPlanId 都指向最顶层的父计划。
	 * In both main and sub-plans, rootPlanId always points to the top-level parent plan.
	 * @return 计划的最外层父计划ID / The root plan ID
	 */
	String getRootPlanId();

	/**
	 * 设置计划的最外层父计划ID（rootPlanId）。
	...
	/**
	 * 获取当前计划ID（currentPlanId）。
	 * currentPlanId 可能等于 rootPlanId（主计划），也可能等于由父计划产生的子计划ID。
	 * currentPlanId may be equal to rootPlanId (for the main plan), or may be a sub-plan ID generated by the parent plan.
	 * @return 当前计划ID / The current plan ID
	 */
	String getCurrentPlanId();

	/**
	 * 设置当前计划ID（currentPlanId）。
	 ...
	 * @param onlyCompletedAndFirstInProgress 当为true时，只输出所有已完成的步骤和第一个进行中的步骤 / If true, only output all completed steps and the first in-progress step
