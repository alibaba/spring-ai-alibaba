...
public class CoordinatorToolRegistry {
	...
	private static final Logger log = LoggerFactory.getLogger(CoordinatorToolRegistry.class);
	...
	public Result<Map<String, List<CoordinatorTool>>> loadPublishedToolsWithResult() {
		...
		try {
			...
			return Result.failure(ToolErrorCode.LOAD_FAILED.getCode(), "Failed to load tools: " + e.getMessage());
		}
	}
	...
	public Result<Boolean> publishWithResult(CoordinatorTool tool) {
		if (tool == null) {
			...
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Tool object cannot be empty");
		}

		try {
			log.info("Starting to publish tool: {} to endpoint: {}", tool.getToolName(), tool.getEndpoint());

			// Call MCP server for dynamic registration
			boolean success = mcpServer.registerCoordinatorTool(tool);

			if (success) {
				log.info("Successfully published tool: {} to endpoint: {}", tool.getToolName(), tool.getEndpoint());
				return Result.success(true, "Tool published successfully");
			}
			else {
				log.error("Failed to publish tool: {} to endpoint: {}", tool.getToolName(), tool.getEndpoint());
				return Result.failure(ToolErrorCode.PUBLISH_FAILED.getCode(), "MCP server registration failed");
			}
		}
		catch (Exception e) {
	...
	public Result<Boolean> publishWithResult(CoordinatorToolEntity entity) {
		if (entity == null) {
			...
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Tool entity object cannot be empty");
		}

		try {
			log.info("Starting to publish tool entity: {} to endpoint: {}", entity.getToolName(), entity.getEndpoint());
	...
	public Result<Integer> publishAllWithResult(List<CoordinatorTool> tools) {
		if (tools == null || tools.isEmpty()) {
			...
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Tool list cannot be empty");
		}
	 ...
	 * @return Publication result
	 */
	public Result<Integer> publishAllEntitiesWithResult(List<CoordinatorToolEntity> entities) {
		if (entities == null || entities.isEmpty()) {
			log.warn("Tool entity list is empty, cannot publish");
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Tool entity list cannot be empty");
		}
	...
	public Result<Boolean> unpublishWithResult(String toolName, String endpoint) {
		if (toolName == null || toolName.trim().isEmpty()) {
			...
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Tool name cannot be empty");
		}

		if (endpoint == null || endpoint.trim().isEmpty()) {
			log.warn("Endpoint address is empty, cannot unpublish");
			return Result.failure(ToolErrorCode.INVALID_PARAMETER.getCode(), "Endpoint address cannot be empty");
		}
		...
		try {
			...
			if (success) {
				...
				return Result.failure(ToolErrorCode.UNPUBLISH_FAILED.getCode(), "MCP server unregistration failed");
			}
		}
		catch (Exception e) {
	...
	public Result<Boolean> unpublishWithResult(CoordinatorTool tool) {
		...
		try {
			log.info("Starting to unpublish tool: {} from endpoint: {}", tool.getToolName(), tool.getEndpoint());

			Result<Boolean> result = unpublishWithResult(tool.getToolName(), tool.getEndpoint());

			if (result.isSuccess()) {
				log.info("Successfully unpublished tool: {} from endpoint: {}", tool.getToolName(), tool.getEndpoint());
				return Result.success(true, "Tool unpublished successfully");
			}
			else {
				log.error("Failed to unpublish tool: {} from endpoint: {}", tool.getToolName(), tool.getEndpoint());
				return result;
			}
		}
		catch (Exception e) {
