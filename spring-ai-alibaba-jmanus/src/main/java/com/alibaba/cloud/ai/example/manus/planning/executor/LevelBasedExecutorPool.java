...
/**
 * Level-based executor common pool that manages thread pools by depth level.
 * Each depth level has its own thread pool to execute tasks at that hierarchy level.
 * Maximum depth level is 10.
 */
@Component
public class LevelBasedExecutorPool {
    ...
    /**
     * Get or create an executor for the specified depth level with custom pool configuration
    ...
    private ExecutorService createLevelPool(int depthLevel, int corePoolSize, int maxPoolSize, int queueCapacity) {
        ...
        log.info("Created executor pool for depth level {}: {} (core: {}, max: {}, queue: {})", 
                depthLevel, poolName, corePoolSize, maxPoolSize, queueCapacity);
