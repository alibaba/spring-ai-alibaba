...
/**
 * ConfigurableDynaAgent - A flexible agent that allows passing tool lists
 * dynamically
 * This agent can be configured with different tool sets at runtime and extends
 * DynamicAgent
 * to inherit all the core functionality while adding configurable tool
 * management.
 */
public class ConfigurableDynaAgent extends DynamicAgent {
     ...
     * @param availableToolKeys        List of available tool keys (can be
     *                                 null/empty)
    ...
    /**
     * Set the available tool keys for this agent
     * This allows overriding the default tools with user-selected tools
     * 
    ...
    public String getToolConfigurationSummary() {
        return String.format("Agent '%s' has %d tools: %s",
                getName(),
                availableToolKeys.size(),
