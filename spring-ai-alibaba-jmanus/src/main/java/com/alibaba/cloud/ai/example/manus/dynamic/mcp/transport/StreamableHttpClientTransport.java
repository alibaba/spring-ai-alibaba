 ...
 * <p>
 * This transport implements a streamable HTTP connection using WebClient and reactive streams.
 * It supports bidirectional communication by maintaining separate input and output message flows.
 * </p>
 */
				...
				if (requestHandler != null && messageObj != null) {
					...
					try {
						requestHandler.apply(Mono.just(messageObj)).subscribe(
							result -> logger.info("=== requestHandler 处理完成: {} ===", result),
							error -> logger.error("=== requestHandler 处理出错: {} ===", String.valueOf(error), error),
