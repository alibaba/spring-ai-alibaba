...
public class StateGraphTest {
	...
	void testValidation() throws Exception {
		...
		exception = assertThrows(GraphStateException.class,
				() -> workflow.addConditionalEdges("agent_1", AsyncEdgeAction.edge_async(state -> "agent_3"), CollectionsUtils.mapOf()));
		System.out.println(exception.getMessage());
	...
	void testWithAppenderDeprecated() throws Exception {
		...
		assertEquals(3, result.get().messages().size());
		assertEquals(3, result.get().steps());
		assertIterableEquals(CollectionsUtils.listOf("message1", "message2", "message3"), result.get().messages().values());
