...
public class StateGraphPersistenceTest {
	...
	static class MessagesState extends AgentState {
		...
		static Map<String, Channel<?>> SCHEMA = CollectionsUtils.mapOf("messages", AppenderChannel.<String>of(ArrayList::new));
	...
	public void testCheckpointInitialState() throws Exception {
		...
		saver.put(runnableConfig, Checkpoint.builder().state(newState).nodeId("agent_1").nextNodeId(StateGraph.END).build());
		app = workflow.compile(compileConfig);
		initState = app.cloneState(app.getInitialState(inputs, runnableConfig));
		...
		StateGraph<MessagesState> workflow = new StateGraph<>(MessagesState.SCHEMA, MessagesState::new)
			.addNode("agent", AsyncNodeAction.node_async(state -> {
				...
				if (lastMessage.contains("bartolo")) {
					return CollectionsUtils.mapOf("messages", "Hi bartolo, nice to meet you too! How can I assist you today?");
				}
			...
			.addNode("tools", AsyncNodeAction.node_async(state -> CollectionsUtils.mapOf("messages", "temperature in Napoli is 30 degree")))
			.addEdge(StateGraph.START, "agent")
			.addConditionalEdges("agent", AsyncEdgeAction.edge_async(
					state -> state.lastMessage().filter(m -> m.equals("tool_calls")).map(m -> "tools").orElse(StateGraph.END)),
